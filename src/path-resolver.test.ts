...
describe("PathResolver", () => {
  ...
  describe("resolveImportPath", () => {
    ...
    test("should resolve alias imports", async () => {
      // Mock existsSync
      const { existsSync } = await import("fs");
      (existsSync as any).mockImplementation((path: any) => {
        const pathStr = path.toString();
        return (
          pathStr === "/project/src/utils/helper.ts" ||
          pathStr === "/project/stores/main.ts"
        );
      });
    ...
    test("should handle imports without extensions", async () => {
      ...
      (existsSync as any).mockImplementation((path: any) => {
        const pathStr = path.toString();
        return pathStr === "/project/src/utils/helper.ts"; // .ts file exists
      });
    ...
    test("should handle index file imports", async () => {
      // Mock existsSync to simulate index file
      const { existsSync } = await import("fs");
      (existsSync as any).mockImplementation((path: any) => {
        const pathStr = path.toString();
        return pathStr === "/project/src/utils/index.ts";
      });
